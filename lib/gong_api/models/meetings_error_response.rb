...
module GongAPI
  class MeetingsErrorResponse
    # Error messages.
    attr_accessor :errors

    # Link to Gong where a Gong administrator can fix the issue
    attr_accessor :fix_url

    # Link to Gong help center.
    ...
    attr_accessor :user_facing_error
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `GongAPI::MeetingsErrorResponse` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `GongAPI::MeetingsErrorResponse`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'user_facing_error')
        self.user_facing_error = attributes[:'user_facing_error']
      end
    end
