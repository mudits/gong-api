...
module GongAPI
  class Tracker
    ...
    # A list of details for each occurrence of Tracker phrases.
    attr_accessor :occurrences

    # A list of occurrence counters for each specific phrase within the tracker (e.g., Amazon, Walmart).
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `GongAPI::Tracker` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `GongAPI::Tracker`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      type_validator = EnumAttributeValidator.new('Object', ['KEYWORD', 'SMART'])
    ...
    def type=(type)
      validator = EnumAttributeValidator.new('Object', ['KEYWORD', 'SMART'])
    ...
    def hash
      [count, id, name, occurrences, phrases, type].hash
    end
