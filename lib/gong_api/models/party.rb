  ...
  class Party
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `GongAPI::Party` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `GongAPI::Party`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      affiliation_validator = EnumAttributeValidator.new('Object', ['External', 'Internal', 'Unknown'])
      return false unless affiliation_validator.valid?(@affiliation)
      true
    end

    # Custom attribute writer method checking allowed values (enum).
    # @param [Object] affiliation Object to be assigned
    def affiliation=(affiliation)
      validator = EnumAttributeValidator.new('Object', ['External', 'Internal', 'Unknown'])
      unless validator.valid?(affiliation)
        fail ArgumentError, "invalid value for \"affiliation\", must be one of #{validator.allowable_values}."
      end
      @affiliation = affiliation
    end
    ...
    def hash
      [affiliation, context, email_address, id, methods, name, phone_number, speaker_id, title, user_id].hash
    end
