  ...
  class SpeechSegment
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `GongAPI::SpeechSegment` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `GongAPI::SpeechSegment`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'party_ids')
        if (value = attributes[:'party_ids']).is_a?(Array)
          self.party_ids = value
        end
      end
    ...
    def list_invalid_properties
      ...
      if @from_time.nil?
        invalid_properties.push('invalid value for "from_time", from_time cannot be nil.')
      end

      if @party_ids.nil?
        invalid_properties.push('invalid value for "party_ids", party_ids cannot be nil.')
      end

      if @to_time.nil?
        invalid_properties.push('invalid value for "to_time", to_time cannot be nil.')
      end
    ...
    def hash
      [from_time, party_ids, to_time].hash
    end
