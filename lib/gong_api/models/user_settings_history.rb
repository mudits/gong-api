  ...
  class UserSettingsHistory
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `GongAPI::UserSettingsHistory` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `GongAPI::UserSettingsHistory`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'setting')
    ...
    def valid?
      setting_validator = EnumAttributeValidator.new('Object', ['active', 'emailsImported', 'nonRecordedMeetingsImported', 'preventEmailImport', 'preventWebConferenceRecording', 'webConferencesRecorded'])
      return false unless setting_validator.valid?(@setting)
      true
    end

    # Custom attribute writer method checking allowed values (enum).
    # @param [Object] setting Object to be assigned
    def setting=(setting)
      validator = EnumAttributeValidator.new('Object', ['active', 'emailsImported', 'nonRecordedMeetingsImported', 'preventEmailImport', 'preventWebConferenceRecording', 'webConferencesRecorded'])
      unless validator.valid?(setting)
        fail ArgumentError, "invalid value for \"setting\", must be one of #{validator.allowable_values}."
      end
      @setting = setting
    end
