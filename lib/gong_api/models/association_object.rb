...
module GongAPI
  class AssociationObject
    # CRM account Id.
    attr_accessor :account_crm_id

    # CRM deal Id, if deal/opportunity was associated.
    attr_accessor :deal_crm_id
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `GongAPI::AssociationObject` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `GongAPI::AssociationObject`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'account_crm_id')
        self.account_crm_id = attributes[:'account_crm_id']
      end

      if attributes.key?(:'deal_crm_id')
        self.deal_crm_id = attributes[:'deal_crm_id']
      end
    end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          account_crm_id == o.account_crm_id &&
          deal_crm_id == o.deal_crm_id
    end
    ...
    def hash
      [account_crm_id, deal_crm_id].hash
    end
