...
module GongAPI
  class RequestStatusResponse
    ...
    attr_accessor :total_success_count
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `GongAPI::RequestStatusResponse` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `GongAPI::RequestStatusResponse`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'total_error_count')
        self.total_error_count = attributes[:'total_error_count']
      end

      if attributes.key?(:'total_success_count')
        self.total_success_count = attributes[:'total_success_count']
      end
    end
    ...
    def valid?
      status_validator = EnumAttributeValidator.new('Object', ['DONE', 'FAILED', 'IN_PROGRESS', 'PENDING'])
    ...
    def status=(status)
      validator = EnumAttributeValidator.new('Object', ['DONE', 'FAILED', 'IN_PROGRESS', 'PENDING'])
    ...
    def ==(o)
      ...
      self.class == o.class &&
          ...
          status == o.status &&
          total_error_count == o.total_error_count &&
          total_success_count == o.total_success_count
    end
    ...
    def hash
      [errors, request_id, status, total_error_count, total_success_count].hash
    end
