...
module GongAPI
  class InteractionStats
    attr_accessor :person_interaction_stats
    ...
    def self.attribute_map
      {
        :'person_interaction_stats' => :'personInteractionStats',
    ...
    def self.openapi_types
      {
        :'person_interaction_stats' => :'Object',
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `GongAPI::InteractionStats` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `GongAPI::InteractionStats`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'person_interaction_stats')
        if (value = attributes[:'person_interaction_stats']).is_a?(Array)
          self.person_interaction_stats = value
        end
      end
    ...
    def hash
      [person_interaction_stats, user_email_address, user_id].hash
    end
