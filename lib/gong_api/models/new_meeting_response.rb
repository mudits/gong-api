...
module GongAPI
  class NewMeetingResponse
    ...
    attr_accessor :additional_invitees
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `GongAPI::NewMeetingResponse` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `GongAPI::NewMeetingResponse`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'additional_invitees')
        if (value = attributes[:'additional_invitees']).is_a?(Array)
          self.additional_invitees = value
        end
      end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          additional_invitees == o.additional_invitees &&
    ...
    def hash
      [additional_invitees, meeting_id, meeting_url, request_id].hash
    end
