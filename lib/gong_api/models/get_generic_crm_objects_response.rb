...
module GongAPI
  class GetGenericCrmObjectsResponse
    # crmObjects
    attr_accessor :crm_objects_map
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `GongAPI::GetGenericCrmObjectsResponse` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `GongAPI::GetGenericCrmObjectsResponse`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'crm_objects_map')
        if (value = attributes[:'crm_objects_map']).is_a?(Hash)
          self.crm_objects_map = value
        end
      end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          crm_objects_map == o.crm_objects_map &&
    ...
    def hash
      [crm_objects_map, request_id].hash
    end
