...
module GongAPI
  class CallTranscripts
    ...
    attr_accessor :call_transcripts
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `GongAPI::CallTranscripts` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `GongAPI::CallTranscripts`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'call_transcripts')
        if (value = attributes[:'call_transcripts']).is_a?(Array)
          self.call_transcripts = value
        end
      end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          call_transcripts == o.call_transcripts &&
          records == o.records &&
